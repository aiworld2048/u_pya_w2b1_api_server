...
class WorldEntertainmentSlotGameSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $jsonPath = base_path('app/Console/Commands/data/world_entertainment_slot.json');
            ...
            foreach ($data['provider_games'] as $game) {
                if (isset($game['status']) && $game['status'] === 'ACTIVATED') {
                    DB::table('game_lists')->insert([
                        ...
                        'status' => $game['status'],
                        'provider' => 'WorldEntertainmentSlotGame',
                        'game_list_status' => 1,
                        'created_at' => $now,
                        'updated_at' => $now,
                    ]);
